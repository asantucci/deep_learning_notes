...
\begin{table}[h]
  ...
  \end{tabular}
\end{center}
\caption{Suppose boba is word $\vec w$ and ice-cream is word $\vec v$, and that we wish to calculate the Euclidean distance between these two word-vectors. This is given by $d(\vec v,\vec w) = \sqrt{(1-0)^2 + (6-4)^2 + (8-6)^2} = \sqrt{1 + 4 + 4} = \sqrt{9} = 3$.}
\end{table}
...
\paragraph{Cosine similarity} Another type of similarity function is cosine similarity. Euclidean distance can be problematic; to see why, consider the following example.
