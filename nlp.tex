...
Notice in the table above that it's of dimension $n \times n$. But this means we have to have a preceding word (and corresponding part of speech tag) in order to predict the next word. Unfortunately, with this set-up we cannot predict the first word in a document, since there is no preceding word! What we can do then is introduce an initial state $\pi$ and include these probabilities in the table in the form of an $(n+1) \times n$ table.
...
\begin{table}[h]
  \begin{center}
    ...
    \caption{Here, we encode an initial state such that we can predict the first word (or corresponding part of speech) in a document.}
  \end{center}
\end{table}
...
\begin{figure}[h]
  \begin{center}
    ...
    \end{tikzpicture}
    \caption{Here, we describe hidden states with a background shade of \color{blue!80}{blue} \color{black} and observable states with \color{gray!80}{gray}.\color{black} The interpretation here is that if the current parts of speech is a verb, then there is a 50\% chance that the next (observable) word will be ``eat''.}
  \end{center}
\end{figure}
