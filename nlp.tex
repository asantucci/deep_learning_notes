...
\section{Neural Networks for Sentiment Analysis}\vspace{.1pt} \hrule height 2pt \smallskip \renewcommand{\arraystretch}{1}% Tighter
...
\begin{figure}[h]   \centering
  ...
  \begin{tikzpicture}[scale=0.25]
    \node (x0) at (-2, 2) {$x_0$};
    \node [below = .5 cm of x0] (x1) {$x_1$};
    \node [below = .5 cm of x1] (x2) {$x_2$};
    \node [below = .5 cm of x2] (inputvdots) {$\vdots$};
    \node [below = .5 cm of inputvdots] (xn) {$x_n$};
    \node [draw,circle,right = 1cm of x1,fill=blue!20] (n11) {};
    \node [draw,circle,below = .5 cm of n11,fill=blue!20] (n12) {};
    \node [draw,circle,below = .5cm of n12,fill=blue!20] (n13) {};
    \node [draw,circle,below = .5cm of n13,fill=blue!20] (n14) {};
    \node [draw,circle,below = .5cm of n14,fill=blue!20] (n15) {};
    \node [draw,circle,right = 1 cm of n11,fill=blue!20] (n21) {};
    \node [draw,circle,below = .5 cm of n21,fill=blue!20] (n22) {};
    \node [draw,circle,below = .5cm of n22,fill=blue!20] (n23) {};
    \node [draw,circle,below = .5cm of n23,fill=blue!20] (n24) {};
    \node [draw,circle,below = .5cm of n24,fill=blue!20] (n25) {};
    \node [draw,circle,right = 1cm of n22,fill=green!20] (n31) {};
    \node [draw,circle,below = .5cm of n31,fill=green!20] (n32) {};
    \node [draw,circle,below = .5cm of n32,fill=green!20] (n33) {};
    \node at (1, -20) {$W^{[1]}$};
    \node at (7, -20) {$W^{[2]}$};
    \node at (13, -20) {$W^{[3]}$};
...
\begin{figure}[h]   \centering
  \label{fig: neuralnetforsentimentanalysis}
  \begin{tikzpicture}[scale=0.25]
    \node (x1) at (-2, 1) {$x_1$};
    \node [below = .5 cm of x1] (x2) {$x_2$};
    \node [below = .5 cm of x2] (inputvdots) {$\vdots$};
    ...
    % \draw[->] (n33) to (outputneuron);
    % \draw[->] (outputneuron) to (yhat);
    \draw[fill=white] (3, -15) rectangle (5, 3);
    \node[rotate=90] at (4, -8.5) {Embedding};
    \draw[->,dashed,ultra thick] (n31) -- ++(5, 0) node[right] {Positive (1)};
    \draw[->,dashed,ultra thick] (n32) -- ++(5, 0) node[right] {Negative (0)};
  \end{tikzpicture}
...
\begin{figure}
  \begin{center}
    \begin{tikzpicture}
      \node[draw] (tweet) at (0, 0) {Tweet: ``This movie was almost good''};
      \node[below=5mm of tweet] (initrep) {[\color{purple!80}700 680 720 20 55 \color{black}]};
      \node[below=5mm of initrep] (padding) {[\color{purple!80} 700 680 720 20         55 \color{orange!80} 0 0 0 0 0 0 0 \color{black}]};
\draw[->,ultra thick] (tweet) -- (initrep);
\draw[->,ultra thick] (initrep) -- (padding);
\end{tikzpicture}
...
\begin{itemize}
\item \href{https://trax-ml.readthedocs.io/en/latest/}{Trax documentation}.
\item \href{https://github.com/google/trax}{Trax source code}.
\end{itemize}

\paragraph{Trax layers} A \emph{serial} model is cmoposed of a series of layers, which is the basic class in Trax. We now forray into how classes work and their methods. In Python, a class is a way to define common properties and methods for similar objects; i.e. classes define common variables and behavior for the object associated with them. E.g., let's say that we have different colors, so we create a \texttt{color} class. Perhaps our class has common data attributes such as \texttt{r, g, b} for every item defined the class, and some methods associated with it.

\begin{figure}[h]
  \begin{center}
    \begin{tikzpicture}
      \node[draw] (colorclass) at (0,0) {Color Class};
      \node[draw, above right = 1mm and 12mm of colorclass, color = gray!50] (params) {Parameters (r, g, b)};
      \node[draw, below right = 1mm and 12mm of colorclass, color = gray!50] (methods)      {Methods};
      \draw[->] (colorclass) -- ++(2, 0) -| ++(0,0.75) |- (params); 
      \draw[->] (colorclass) -- ++(2, 0) -| ++(0,-0.7) |- (methods);
      \node[draw, below left = 12mm and 8mm of colorclass,color=orange!80]       (orange) {Orange};
\node[draw, below = 12mm of colorclass, color=blue!80] (blue) {Blue};
\node[draw, below right = 12mm and 8mm of colorclass, color=purple!50] (magenta) {Magenta};
\draw[->] (colorclass) -- (blue);
\draw[->] (colorclass) -- ++(0,-1) -| (orange);
\draw[->] (colorclass) -- ++(0,-1) -| (magenta);
\node[draw,dashed,color=orange!80,below=5mm of orange] {\color{black} $\substack{r = 230 \\ g = 145 \\ b = 56}$};
\node[draw,dashed,color=blue!80,below=5mm of blue] {\color{black} $\substack{r = 61 \\ g = 133 \\ b = 198}$};
\node[draw,dashed,color=orange!80,below=5mm of magenta] {\color{black} $\substack{r = 166 \\ g = 77 \\ b = 121}$};
\end{tikzpicture}
\caption{Each of the instances from your class in this case are the colors orange, blue, and magenta. They each possess a specific set of RGB values.}
  \end{center}
\end{figure}
...
\section{Recurrent Neural Networks for Language Modeling}\vspace{.1pt} \hrule height 2pt \smallskip \renewcommand{\arraystretch}{1}% Tighter
...
\begin{figure}[h]
  \begin{center}
    \begin{tikzpicture}[scale=3/4]
      \node[minimum width = 5cm, text width = 4.8cm, anchor = west] (e1) at (0,2) {I saw the game of soccer};
      \node[minimum width = 5cm, text width = 4.8cm, anchor = west] (e2) at (0,1) {I saw the soccer         game};
      \node[minimum width = 5cm, text width = 4.8cm, anchor = west] (e3) at (0,0) {I saw the soccer         match};
      \node[minimum width = 5cm, text width = 4.8cm, anchor = west] (e4) at (0,-1) {Saw I the game of soccer};
      \draw [decorate,decoration={brace,amplitude=10pt,mirror},xshift=-4pt,yshift=0pt]
(0, 2) -- (0, -1);
\draw[->, ultra thick] (-2,0.5) -- (-1, 0.5);
      \node[minimum width = 5cm, text width = 4.8cm, anchor = east] (french) at                   (-2,0.5) {J'ai vu le match de foot};
      \node[below=2mm of french] {\faSoccerBallO};
    \end{tikzpicture}
...
\paragraph{How does an RNN work?} To see how an RNN works, let's look at just the second sentence from our previous example, ``I called her but she did not \uline{\hspace{15pt}}''.
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
...
\begin{figure}[h]
  ...
  \end{tikzpicture}
  \caption{\footnotesize At each time-step, we take in an input $X^{T_i}$, a hidden state $h$, and we make a prediction $\hat y^{T_i}$. Additionally, we propagate a new hidden state to the next time step. The hidden state at each time step is computed with an activation function $g$ with arguments equalt to the product between a parameter matrix $W_{aa}$ and the previous hidden state $a^{\langle T_{i-1} \rangle}$.}
\end{figure}
...
\begin{figure}
  \begin{center}
    \begin{tikzpicture}
      \draw[fill=blue!20] (-3.25, -1.75) rectangle (2.5, 1.5);
      % Stack on non-linearities leading to output(s).
      \node[draw,fill=white] (nonlinearityabove) at (1, 1) {\texttt{sigmoid}};
      \node[draw,fill=white,below=7mm of nonlinearityabove] (nonlinearitybelow)       {\texttt{sigmoid}};
      \draw[->] (nonlinearitybelow) -- (nonlinearityabove) node[midway, right]       {$W_{ya}$};
      % Outputs themselves.
      \node[above=7mm of nonlinearityabove] (yhat) {\color{green!80!black} $\hat y^{\langle t \rangle}$};
      \draw[->, ultra thick, green!80!black] (nonlinearityabove) -- (yhat);
      \node[right=12mm of nonlinearitybelow] (activationoutput) {\color{orange!80} $a^{\langle t           \rangle }$};
      \draw[->, ultra thick, orange!80] (nonlinearitybelow) -- (activationoutput);
      % Inputs.
      \node[draw,fill=white,circle,left=12mm of nonlinearitybelow] (plus) {$+$};
      \draw[->] (plus) -- (nonlinearitybelow);
      \node[below=12mm of plus] (xinput) {\color{purple!80} $x^{\langle t \rangle}$};
      \draw[->, ultra thick, purple!80] (xinput) -- (plus) node[midway, right]       {\color{black} $W_{ax}$};
      \node[left=15mm of plus] (activationinput) {\color{orange!80} $a^{\langle           t - 1 \rangle }$};
      \draw[->, ultra thick,orange!80] (activationinput) -- (plus) node[midway,       above] {\color{black} $W_{aa}$};
\end{tikzpicture}
\caption{This is an example diagram depicting the order of operations in an arbitrary recurrent neural network layer. The inputs to the cell are the current variable $x^{\langle t  \rangle}$ and the inputs from the previous layer $a^{\langle t - 1 \rangle}$. To get the current hidden state, first, we have toget the product of $W_{ax} x^{\langle t \rangle}$ and $W_{aa} a^{\langle t-1 \rangle}$, and then sum the resulting vectors element-wise. Next, we pass the resulting vector through an activation function, and with the resulting value we can both compute a prediction (by first multiplying the hidden state $a^{\langle t \rangle }$ with the learnable parameters $W_{ya}$ and passing through a non-linearity) and also the information to propagate to the subsequent layer.}
  \end{center}
\end{figure}

The basic summary is that the hidden states propagate information through time, and a basic recurrent unit has two inputs: $a^{\langle t - 1 \rangle}$ and $x^{\langle t \rangle}$.

\section{LSTMs and Named Entity Recognition}\vspace{.1pt} \hrule height 2pt \smallskip \renewcommand{\arraystretch}{1}% Tighter
\section{Siamese Networks}\vspace{.1pt} \hrule height 2pt \smallskip \renewcommand{\arraystretch}{1}% Tighter
