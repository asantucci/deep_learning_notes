...
operation we end up with a $(n+2p-f+1) \times (n+2p-f+1)$ output image. If we want
...
\begin{figure}[h]   \centering
  \begin{tikzpicture}[scale=0.7]       \draw[step=1cm] (0, 0) grid (7, 7);
...
\subsubsection{Convolutions over Volume} Let's consider a convolution over 3D volumes. Suppose we have a $6 \times 6$ matrix
...
\paragraph{Dimensions summary} If you have an $n \times n \times n_c$ where $n_c$ denotes the number of channels,
...
\paragraph{Though exercise} If we have 10 filters that are each $3 \times 3 \times 3$ in one layer of a neural network, how many
...
Next, consider the weights. We've mentioned above that th dimension of a single filter is $f^{[\ell]} \times f^{[\ell]} \times n_c^{[\ell-1]}$, so to get the total number of weights we need 
\[
f^{[\ell]} \times f^{[\ell]} \times n_c^{[\ell-1]} \times n_c^{[\ell]}
\]
...
\subsubsection{Deep convolutional Neural Network} Suppose we have an image and we want to perform image recognition, e.g.
...
computation. The same formulae apply for dimensions, where the size of he subsequent layer is given by $\floor{\frac{n+2p - f}{s} + 1}$. If you have an image with multiple channels, then the output of the max-pooling operation will have just as many channels.
...
\subsection{CNN Example} We now have the building blocks for constructing a full convolutional neural network, and so we next
...
to deeper layers. The pattern of $\texttt{Conv} \leadsto \texttt{Pool} \leadsto \texttt{Conv} \leadsto \texttt{Pool} \leadsto \texttt{FC} \leadsto \texttt{FC} \leadsto \texttt{FC} \leadsto \texttt{softmax}$ is very common in computer vision.
...
\begin{table}[h]
  ...
  \texttt{Conv1} ($f=5,s=1$) & $(28, 28, 8)$ & $4,704$ & $456$ \\
...
\begin{itemize}   \item Components of the network involving Pooling have no parameters to learn.
...
\begin{itemize}   \item Parameter sharing, and
