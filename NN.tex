...
\begin{figure}[h]
  ...
  \end{tikzpicture}
  \caption{\footnotesize Consider two different input sentences, which have different outputs for a named entity recognition task.
\begin{itemize}\item He said ``Teddy bears are on sale!''
\item He said, ``Teddy Roosevelt was a great President!''\end{itemize}
In order to figure out whether the third word Teddy is part of a person's name, it's not enough to just look at the first part of the sentence; to tell if $y^{\langle 3 \rangle}$ should be a zero or one, we need more information. This problem persists in any unidirectional or forward direction only RNN, no matter whether we use a standard RNN block, GRU's, or LSTM's.}
\end{figure}
