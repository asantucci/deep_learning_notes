...
\begin{align}   z^{[\ell+1]} &= W^{[\ell+1]} a^{[\ell]} + b^{[\ell+1]} \nonumber \\
  a^{[\ell+1]} &= g(z^{[\ell+1]}) \nonumber \\ 
  z^{[\ell+2]} &= W^{[\ell+2]} a^{[\ell+1]} + b^{[\ell+2]} \nonumber \\
  \label{eq: skipconold}
...
\begin{figure}[h]   \centering
  ...
  \end{tikzpicture}
  \caption{A skip connection which bypasses the ``main path''.} 
\end{figure}

And so our updated network diagram now looks like
...
\begin{tikzpicture}
  \begin{axis}[     
    ...
    \addplot [purple, domain = 0:2] {1/2*pow(x-1, 2)+1/2} node[right] {Practice}; 
  \end{axis} 
...
In contrast, with a ResNet we avoid problems of exploding and vanishing gradients since we are able to feed activations directly
...
\subsubsection{Why do residual networks work?} We've seen previously that obtaining low training error
...
\begin{figure}   \centering
  \begin{tikzpicture}     \node at (0,0) {x};
    \draw[->] (0.2, 0) -- (1, 0) node[right,draw,rectangle] (bignn) {Big NN};
    \draw[->] (bignn) -- (3.5,0) node[above left] {$a^{[\ell]}$};  
    \draw (4,-.5) rectangle (4.5, .5);
    \foreach \y in {-.25, 0, .25} \node[draw,circle,scale=1/2] at (4.25, \y) {};
    \draw[->] (4.5, 0) -- (5,0);   \end{tikzpicture} \end{figure}
