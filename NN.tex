...
\begin{tikzpicture}
  ...
  \draw[->] (z1) to (a1);
  \node[draw=black,rectangle] (z2) at (12.25, -1.5) {$z^{[2]} = W^{[2]}a^{[1]} + b^{[2]}$};
  \draw[->] (a1) to (z2);
...
\vspace{-.5ex}
\paragraph{Bias - Variance} In Deep Learning, we still talk about bias and variance, but we don't emphasize as much that there is a tradeoff between these two quantities. Andrew considers \emph{bias} to be determined by the training set error relative to how well a human can perform the same task (i.e. \href{https://en.wikipedia.org/wiki/Bayes_error_rate}{Bayes error}), and the \emph{variance} to be determined by the relation to the training set error. 
...
\]
where we're using Frobenius norm $\|W^{[\ell]}\|_F^2 = \sum_{i=1}^{n^{[\ell]}} \sum_{j=1}^{n^{[\ell-1]}} (W_{ij}^{[\ell]})^2$, recalling that $W \in \mathbb R^{n^{[\ell]} \times n^{[\ell - 1]}}$, and that $\frac{\partial}{\partial X} \|X\|_{\textrm{F}}^2 = \frac{\partial}{\partial X} \textrm{Tr}(XX^T) = 2X$. We need to make note that we have a new rule for back-propagation:
\[
...
simply done in order to keep, in expectation, $z^{[4]} = W^{[4]} a^{[3]} + b^{[4]}$ the same. Note that this Boolean array \texttt{d3} is also used in back-propagation. 
...
We begin to standardize our data by first subtracting out the mean: $\mu = \frac{1}{m} \sum_{i=1}^{m} x^{(i)}$, $x -= \mu$.
...
\caption{\footnotesize Data after the mean has been subtracted out from each dimension.}
\end{figure}
...
\caption{\footnotesize Data after rescaling by the variance.}
\end{figure}
...
\begin{algorithm}
  \caption{\texttt{Grad-check}}   \For{each $i$} {
    \tcp{\footnotesize Requires two calls to feed-forward network to evaluate loss with perturbed inputs.}
...
what if $m$ really large, e.g. 50M? This means that we have to churn through a lot of compute before taking a very small step in gradient descent. It turns out that we can often allow our algorithm to learn faster if we allow for gradient descent to start taking place before we're done processing all $m$ training examples.
...
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \node at (0, 0) {\includegraphics{fig/car_driving}};
...
\begin{figure}[h]
  \begin{tikzpicture}
    ...
    \pgfmathsetmacro{\cubey}{1} 
    \pgfmathsetmacro{\cubez}{1}
    \node at (-2, 0) {\includegraphics[scale=0.15]{fig/profile}};
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \node[draw,rectangle] (forget) at (1.5, 0.5) {\footnotesize forget gate};
    \node[draw,rectangle] (update) at (4, 0.5) {\footnotesize update gate};
    \node[draw,rectangle] (tanh)   at (6, 0.5) {$\scriptstyle \tanh$};
    \node[draw,rectangle] (output) at (8, 0.5) {\footnotesize output gate};
    \node at (0.5, -0.5)  {$x^{\langle t \rangle}$};
    \node at (-1,   1)    {$a^{\langle t - 1 \rangle}$};
    \node (ctm1) at (-1,   2.25) {$c^{\langle t - 1 \rangle}$};
    \draw[->,blue!50] plot [smooth] coordinates { (-0.5, 1) (-0.4,                         0.1) (1.25, 0.1) (1.5, 0.2)};
    \draw[blue!50] plot [smooth] coordinates { (0.5, -0.25) (0.75, 0) (0.9,       0.075)};
    \draw[->,blue!50] plot [smooth] coordinates { (0.9, 0.075) (3.5, 0.075)       (4.2, 0.2)};
    \draw[->,blue!50] plot [smooth] coordinates { (3.5, 0.075) (5.5, 0.075) (6,       0.2)};
    \draw[->,blue!50] plot [smooth] coordinates { (5.5, 0.075) (7, 0.075) (8,       .2) };
    \node[draw,circle,scale=0.5] (fttimesctm1) at (1.5, 2.25)  {$\times$};
    \draw[->] (ctm1) -- (fttimesctm1);
    \draw[->] (forget) -- (fttimesctm1);
    \node at (1.2, 1) {$f^{\langle t \rangle}$};
    \node at (3.5, 1) {$i^{\langle t \rangle}$};
    \node at (5.5, 1) {$\tilde c^{\langle t \rangle}$};
    \node at (7.5, 1) {$o^{\langle t \rangle}$};
    \draw (tanh) -- (6, 1.5);
    \node[draw,circle,scale=0.5] (ittimesct) at (4, 1.5) {$\times$};
    \draw[->] (6, 1.5) -- (ittimesct);
    \draw[->] (update) -- (ittimesct);
    \node[draw,circle,scale=0.5] (plus) at (4, 2.25) {$+$};
    \draw[->] (ittimesct) -- (plus);
    \draw[->] (fttimesctm1) -- (plus);
    \draw[->] (plus) -- (11, 2.25) node[above] {$c^{\langle t \rangle}$};
    \node[draw,circle,scale=0.5] (ottimescttahnh) at (8, 1.25) {$\times$};
    \draw[->] (output) -- (ottimescttahnh);
    \node[draw,rectangle,scale=0.5] (tanh2) at (8, 1.85) {$\tanh$};
    \draw[->] (tanh2) -- (ottimescttahnh);
    \draw[->] (8, 2.25) -- (tanh2);
    \draw[->] (ottimescttahnh) -- (11, 1.25) node[above] (at) {$a^{\langle t         \rangle }$};
    \node[draw,rectangle,scale=0.5] (softmax) at (11.5, 1.25) {\footnotesize       softmax};
\draw[->] (softmax) -- (12, 1.25) node[right] {$\hat y^{\langle t \rangle }$};
\end{tikzpicture}
\caption{\footnotesize The computational diagram for a basic LSTM.}
\end{figure}
...
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \node (a0) at (0, 0) {$a^{\langle 0 \rangle}$};
    \foreach \x in {1,...,7} {
      \node[draw,rectangle] (a\x) at (2*\x,0) {$a^{\langle \x \rangle}$};
      \node (x\x) at (2*\x, -1) {$x^{\langle \x \rangle}$};
      \node (y\x) at (2*\x,  1) {$y^{\langle \x \rangle}$};
      \draw[->] (x\x) -- (a\x);
      \draw[->] (a\x) -- (y\x);
    }
    \draw[->] (a0) -- (a1);
    ...
    \draw[->] (a5) -- (a6);
    \draw[->] (a6) -- (a7);
    \node[below = 0.1cm of x1] {He};
    \node[below = 0.1cm of x2] {said,};
    \node[below = 0.1cm of x3] {``Teddy};
    \node[below = 0.1cm of x4] {bears};
    \node[below = 0.1cm of x5] {are};
    \node[below = 0.1cm of x6] {on};
    \node[below = 0.1cm of x7] {sale!''};
  \end{tikzpicture}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    \foreach \x in {1,...,4} {
      \node[draw,rectangle] (af\x) at (3*\x-0.65,0) {$\overrightarrow a^{\langle \x \rangle}$};
      \node[draw,rectangle] (ab\x) at (3*\x+0.65,0) {$\overleftarrow a^{\langle \x \rangle}$};
      \node (x\x) at (3*\x, -1) {$x^{\langle \x \rangle}$};
      \node (y\x) at (3*\x,  1) {$\hat y^{\langle \x \rangle}$};
      \draw[->] (x\x) -- (af\x);
      \draw[->] (x\x) -- (ab\x);
      \draw[->] (af\x) -- (y\x);
      \draw[->] (ab\x) -- (y\x);
    }
    \path[->,orange!50] (af1) edge[bend left] (af2);    
    \path[->,orange!50] (af2) edge[bend left] (af3);    
    \path[->,orange!50] (af3) edge[bend left] (af4);
    \path[->,purple!50] (ab4) edge[bend left] (ab3);
    \path[->,purple!50] (ab3) edge[bend left] (ab2);
    \path[->,purple!50] (ab2) edge[bend left] (ab1);
  \end{tikzpicture}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    \foreach \y in {1, 2, 3} {
      \node (a\y0) at (-0.5, \y) {$a^{[\y]\langle 0 \rangle}$};
    }
    \foreach \x in {1,...,4} {
      \node (x\x) at (2*\x,0) {$x^{\langle \x \rangle}$};
      \node (y\x) at (2*\x,4) {$\hat y^{\langle \x \rangle}$};
    }
    \foreach \x in {1,...,4} {
      \foreach \y in {1, 2, 3} {
        \node[draw,rectangle] (a\y\x) at (2*\x,\y) {$a^{[\y]\langle \x \rangle}$};
}
}
\foreach \y in {1, 2, 3} { \draw[->] (a\y0) -- (a\y1); \node at (0.25,\y-0.25) {\color{orange!50} $\scriptstyle W_a^{[\y]}, b_a^{[\y]}$}; }
\foreach \x in {1, ..., 4} {
  \draw[->] (x\x) -- (a1\x);
  \draw[->] (a3\x) -- (y\x);
}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \draw[->] (a4) -- (a5);
    \draw[->] (a5) -- (a6);
    \node[below = 0.1cm of x1] {Sally};
    \node[below = 0.1cm of x2] {Johnson};
    \node[below = 0.1cm of x3] {is};
    \node[below = 0.1cm of x4] {an};
    \node[below = 0.1cm of x5] {orange};
    \node[below = 0.1cm of x6] {farmer};
    \node (y1) at (2*1,  1) {$1$};
    \node (y2) at (2*2,  1) {$1$};
    \node (y3) at (2*3,  1) {$0$};
    \node (y4) at (2*4,  1) {$0$};
...
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \draw[->] (0, 0) -- (1, 1) node[right] (u) {$u$};
    \draw[->] (0, 0) -- (1, 0) node[right] (v) {$v$};
    % \pic[<->, "$\theta$", angle eccentricity = 1.5] {angle = u--v};
    \node at (0.5, 0.25) {$\phi$};
    \draw[->] (3, 0) -- (10, 0) node[right] {$\phi$};
    \draw[->] (3, -1) -- (3, 1.5) node[left]  {$\cos(\phi)$};
    \draw[domain=3:9.28,smooth,variable=\y,orange!75]  plot     ({\y},{cos(deg(\y-3))});
\node at (2.8, 1) {1};
\node at (2.8,-1) {-1};
\end{tikzpicture}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \foreach \y in {2, 1, 0, -2} {
      \node[draw,circle,scale=0.5] (neuron\y) at (3, \y);
      \draw[->] (ej) -- (neuron\y);
    }
    \node at (3, -1) {$\vdots$};
    \node at (4, 2) {juice?};
    \node at (4, 1) {king?};
    \node (vocabsize) at (5, 0) {$|V|$};
    \draw[->] (vocabsize) -- (5, 2);
    \draw[->] (vocabsize) -- (5,-2);
  \end{tikzpicture}
\end{figure}
...
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \draw[->] (0,0) -- (2,0) node[right] {gender};
    \draw[->] (0,0) -- (0,2) node[above left] {royal};
    \draw[->,orange!50] (0,0) -- (2,1) node[right] {$e_{w,1}$};
    \draw[->,orange!50] (0,0) -- (1,2) node[above left] {$e_{w,2}$};
  \end{tikzpicture}
\end{figure}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \foreach \x in {1,2,3,4} {
      \node[draw,rectangle] (a\x) at (2*\x,0) {$a^{\langle \x \rangle}$};
      \node (e\x) at (2*\x,-1) {$e_{j_\x}$};
      \node (E\x) at (2*\x,-2) {$E$};
    }
    \node (a5) at (2*5,0) {$\ldots$};
    \node[draw,rectangle] (a10) at (2*6,0) {$a^{\langle 10 \rangle}$};
    \node (e10) at (2*6,-1) {$e_{j_{10}}$};
    \node (E10) at (2*6,-2) {$E$};
    \draw[->] (a0) -- (a1);
    \draw[->] (a1) -- (a2);
    \draw[->] (a2) -- (a3);
    ...
    \draw[->] (a4) -- (a5);
    \draw[->] (a5) -- (a10);
    \node[below = 0.5cm of E1] (x1) {Completely};
    \node[below = 0.5cm of E2] (x2) {lacking};
    \node[below = 0.5cm of E3] (x3) {in};
    \node[below = 0.5cm of E4] (x4) {good};
    % \node[below = 0.5cm of E5] (x5) {$\ldots$};
    \node[below = 0.5cm of E10](x10) {ambience};
    \foreach \x in {1, 2, 3, 4,10} {
      \draw[->] (x\x) -- (E\x);
      \draw[->] (E\x) -- (e\x);
      \draw[->] (e\x) -- (a\x);
    }
    \node[draw,rectangle,above=0.5cm of a10] (softmax) {softmax};
    \node[above=0.5cm of softmax] (yhat) {$\hat y$};
    \draw[->] (a10) -- (softmax);
    \draw[->] (softmax) -- (yhat);
  \end{tikzpicture}
...
\end{align*}
where in particular we've chosen pairs of words that differ in gender. We can take the resulting collection of differences and average them (or take an Singular Value Decomposition) to figure out the bias direction(s). The next step is a neutralization step: that is for every word that's \emph{not} definitional, project it to get rid of bias. So, there are some words that intrinsically capture gender, so words like grandmother, grandfather, girl, boy, she, and he; for these words gender is intrinsic in the definition. Whereas there are other words like doctor and babysitter where we want the word embedding associated to be (gender) neutral. For non-definitional words, we'll project them onto the non-bias dimension to reduce this bias aspect from the word embedding. In our simple example, bias is a single dimension and we might have a $k$-dimensional word embedding and so there is a bias axis and another axis which captures the $k-1$ remaining dimensions.

% \begin{figure}[h]
%   \begin{tikzpicture}
%     \draw[->] (-1, 0) -- (1, 0) node[right] {bias (1d)};
%     \draw[->] (0, -1) -- (0, 1) node[above] {non-bias dimension {$k-1$         % dimensional};
%     \node[draw,circle,fill] (babysitter) at (-0.5, 0.5) {babysitter};
%     \node[draw,circle,fill] (grandmother) at (-0.25, -0.1) {grandmother};
%     \node[draw,circle,fill] (girl) at (-0.34, -0.2) {girl};
%     \node[draw,circle,fill] (she) at (-0.34, -0.5) {she};
%   \end{tikzpicture}
% \end{figure}

\subsection{Sequence to Sequence Architectures}
Sequence-to-sequence models are useful for everything from machine translation to speech recognition. We start by learning about basic models and then later learn about concepts like beam search and the attention model.

\subsubsection{Basic models} Suppose we want to translate a French sentence into an English sentence
\[
\textrm{Jane visite l'Afrique en septembre} \leadsto \textrm{Jane is visiting Africa in September.}
\]
As usual, we use $x^{\langle 1 \rangle}, \ldots, x^{\langle 5 \rangle}$ to represent the words in the input sequence, and $y^{\langle 1 \rangle}, \ldots, y^{\langle 6 \rangle}$ to represent the words in the output sequence. How can we train a neural network to learn such a mapping? The ideas here are inspired from \href{https://papers.nips.cc/paper/5346-sequence-to-sequence-learning-with-neural-networks.pdf}{Sutskever et. al.'s} and \href{https://arxiv.org/abs/1406.1078}{Cho et. al.'s} 2014 papers. We first construct an \emph{encoder} RNN network, using either GRU's or LSTM's, by feeding in the French words one at a time.
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \node (a0) at (0,0) {$a^{\langle 0 \rangle}$};
    \node[draw,square] (g1) at (1,0) {};
    \node (x1) at (1,-1) {$x^{\langle 1 \rangle}$};
    \draw[->] (a0) -- (g1);
    \draw[->] (x1) -- (g1);
    \draw[->] (g1) -- (1.5, 0);
    \node (dots) at (1.75, 0) {$\ldots$};
    \node (xtx) at (2.5, -1) {$x^{\langle T_x \rangle}$};
    \node[draw,square] (g2) at (2.5, 0) {};
    \draw[->] (dots) -- (g2);
    \draw[->] (xtx)  -- (g2);
    \draw[->] (g2) -- (4,0);
    \node (encoderoutput) at (3.25, 1.25) {$\substack{\textrm{vector           representation} \\ \textrm{of input sequence}}$};
    \draw[->,dashed] (encoderoutput) -- (3.25, 0);
    \node[draw,square,orange] (gy1) at (4.25,0) {};
    \node[above = 1 mm of gy1] (y1) {$y^{\langle 1 \rangle}$};
    \node[draw,square,orange] (gy2) at (5.5,0) {};
    \node[above = 1 mm of gy2] (y2) {$y^{\langle 2 \rangle}$};
    \node[draw,square,orange] (gy3) at (6.75, 0) {};
    \node[above = 1 mm of gy3] (y3) {$y^{\langle 3 \rangle}$};
    \node[orange] (dotsy) at (7.5, 0) {$\ldots$};
    \node[draw,square,orange] (gyty) at (8.5,0);
    \node[above = 1 mm of gyty] (yty) {$y^{\langle T_y\rangle}$};
    \draw[->,orange] (gy1) -- (gy2);
    \draw[->,orange] (gy2) -- (gy3);
    \draw[->,orange] (gy3) -- (dotsy);
    \draw[->,orange] (dotsy) -- (gyty);
    \draw[->,purple!50] plot [smooth] coordinates { (4.5, 0.8) (4.8, 1.15) (5.1, -1) (5.45, -0.35) };
    \draw[->,purple!50] plot [smooth] coordinates { (5.75, 0.8) (6.05, 1.15)       (6.35, -1) (6.7, -0.35) };
    \draw[->,purple!50] plot [smooth] coordinates { (7.75, -.75) (8, -1) (8.45, -0.35) };
  \end{tikzpicture}
  \caption{\footnotesize We build two separate networks, an encoding and decoding network. The encoding network learns a vector representation for the input sequence. The decoder's task is to ingest the encoding output and then output one word at a time until it an end-of-sequence or end-of-sentence token is output, upon which the decoder stops. As usual, we can take generated tokens from the decoder and feed them into the sequence just like we did when synthesizing text using the language model.}
\end{figure}

One of the most remarkable recent results in deep learning is that this model works, given enough pairs of French and English sentences. An architecture very similar to this also can work for image captioning: given an image, perhaps we want to automatically caption it.

\begin{figure}[h]
  \centering
  \includegraphics[scale=0.15]{fig/cat_sitting_in_chair}
  \caption{\footnotesize Given an image as shown here, perhaps we would like it to be captioned automatically as ``A cat sitting on a chair''. We've seen how we can input an image into a (pre-trained) convolutional neural network in order to learn an encoding or a set of features for the input image. E.g. we could take the AlexNet architecture, remove the final softmax unit, and be left with a 4,096 dimensional feature vector which can be used to represent any input image. This can serve as our \emph{encoder} network. You can then feed \emph{this} feature vector as input into an RNN, whose job it is to generate a caption one word at a time. This works better when the captions are not so long. See Mao's 2014 paper on \href{https://arxiv.org/abs/1412.6632}{Deep Captioning}, and Karpathy's 2015 paper on \href{https://cs.stanford.edu/people/karpathy/cvpr2015.pdf}{Deep Visual Semantic Alignments for Generating Image Descriptions}.}
\end{figure}

\subsubsection{Picking the most likely output sequence} There are some similarities between the sequence-to-sequence machine translation model and the language models that we saw previously, but there are also significant differences; machine translation can be seen as building a \emph{conditional} language model. Language models allow us to estimate the probability of a sentence; we can also use such a model to generate novel sequences of text.
...
\begin{figure}[h]
  \centering
    ...
    \draw[->,purple!50] plot [smooth] coordinates { (5, 3) (5.5, 3) (5.75, 1.5) };
\end{tikzpicture}
\caption{\footnotesize A language model architecture.}
\end{figure}

\begin{figure}[h]
  \centering
    \begin{tikzpicture}
    \node[blue!50] (a0) at (0,0) {$a^{\langle 0 \rangle}$};
    \node[draw,square,blue!50] (g1) at (1,0) {};
    \node[blue!50] (x1) at (1,-1) {$x^{\langle 1 \rangle}$};
    \draw[->,blue!50] (a0) -- (g1);
    \draw[->,blue!50] (x1) -- (g1);
    \draw[->,blue!50] (g1) -- (1.5, 0);
    \node[blue!50] (dots) at (1.75, 0) {$\ldots$};
    \node[blue!50] (xtx) at (2.5, -1) {$x^{\langle T_x \rangle}$};
    \node[draw,square,blue!50] (g2) at (2.5, 0) {};
    \draw[->,blue!50] (dots) -- (g2);
    \draw[->,blue!50] (xtx)  -- (g2);
    \draw[->] (g2) -- (4,0);
%     \node (encoderoutput) at (3.25, 1.25) {$\substack{\textrm{vector           % representation} \\ \textrm{of input sequence}}$};
%     \draw[->,dashed] (encoderoutput) -- (3.25, 0);
    \node[draw,square,orange] (gy1) at (4.25,0) {};
    \node[above = 1 mm of gy1,orange] (y1) {$y^{\langle 1 \rangle}$};
    \node[draw,square,orange] (gy2) at (5.5,0) {};
    \node[above = 1 mm of gy2,orange] (y2) {$y^{\langle 2 \rangle}$};
    \node[draw,square,orange] (gy3) at (6.75, 0) {};
    \node[above = 1 mm of gy3,orange] (y3) {$y^{\langle 3 \rangle}$};
    \node[orange] (dotsy) at (7.5, 0) {$\ldots$};
    \node[draw,square,orange] (gyty) at (8.5,0);
    \node[above = 1 mm of gyty,orange] (yty) {$y^{\langle T_y\rangle}$};
    \draw[->,orange] (gy1) -- (gy2);
    \draw[->,orange] (gy2) -- (gy3);
    \draw[->,orange] (gy3) -- (dotsy);
    \draw[->,orange] (dotsy) -- (gyty);
    ...
    \draw[->,purple!50] plot [smooth] coordinates { (7.75, -.75) (8, -1) (8.45, -0.35) };
\end{tikzpicture}
\caption{\footnotesize The decoder network above is essentially identical to that of a language model architecture, except that instead of always starting with a vector of all zeros it instead has an encoded representation for an input sequence (or image). Instead of modeling the probability of any sentence, we are now modeling the probability of, e.g. the output English translation conditional on some input French sentence. I.e. we're estimating the probability of a translation: e.g. what's the chance that the translation is ``Jane is visiting Africa in September'' conditional on the French input sequence being ``Jane visite l'Afrique en septembre''. Our model is estimating the probability of an English sentence conditional on an input French sentence.}
\end{figure}

If $x$ is our input sentence in French, ``Jane visite l'Afrique en septembre'', our model now tells us what is the probability of different English translations of that French input. We do \emph{not} want to sample outputs at random; if we did sample words from this distribution, $p(y|x)$, we'd get different translations each time (of varying quality), e.g.
\begin{itemize}
\item Jane is visiting Africa in September.
\item Jane is going to be visiting Africa in September.
\item In September, Jane will visit Africa.
\item Her African friend welcomed Jane in September.
\end{itemize}
We'd like to instead select the output sequence that maximizes the conditional probability; the most common method for finding the most probable output sequence is called Beam Search. But, before learning about that it's worthy to consider why a greedy approach wouldn't work: one in which we generate the first word according to the most likely word specified by our conditional language model, and then having picked the first word, we proceed to pick the next word that is most likely conditional on the first, etc. Instead of this approach, we'd like to select the output sequence that maximizes the joint probability of $\Pr(\hat y^{\langle 1 \rangle}, \ldots, \hat y^{\langle T_y \rangle})$. To demonstrate, consider two translations, the first of which is better given our example.
\begin{itemize}
\item Jane is visiting Africa in September.
\item Jane is going to be visiting Africa in September.
\end{itemize}
Since the first one is a better translation, hopefully our machine translation model will suggest that $p(y_1|x) > p(y_2 | x)$; the first sentence is a better and more succinct translation, whereas the second is a bit verbose and has superfluous words. However, if an algorithm has output ``Jane is'' as the first two words, then because ``going'' is a more common English word than ``visiting'', it may turn out that the $\Pr(\textrm{going} | \textrm{Jane is}) > \Pr(\textrm{visiting} | \textrm{Jane is})$. Further, the total number of combinations of words in the output sequence space is exponentially large, since if we have a vocabulary of size $|V|$ and we are contemplating translations up to ten words long, there are $|V|^{10}$ sentences to choose from. Since it's impossible to rank all sentences, the most common thing to do is to use an approximate search.

\subsubsection{Beam Search} In machine translation and audio recognition, we don't simply want to output just \emph{any} random sequence, but rather the best or \emph{most probable}. To make things concrete, let's suppose we're tasked with translating ``Jane visite l'Afrique en Septembre'' from French to English. The beam search algorithm still operates on our English vocabulary, and accepts as input the output from an \emph{encoder} in the form of a vector of floating point values.

\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \node (vocabsize) at (-2,0) {$|V|$};
    \draw[->] (vocabsize) -- (-2, 2);
    \draw[->] (vocabsize) -- (-2,-2);
    \node at (0,0) {$\begin{bmatrix} \textrm{a} \\ \vdots \\ \textrm{in} \\ \vdots \\ \textrm{jane} \\ \vdots \\ \textrm{september} \\ \vdots \\ \textrm{zulu}        \end{bmatrix}$};
  \end{tikzpicture}
\end{figure}

Whereas greedy search will pick the most likely word and move on, what Beam Search does is consider multiple alternatives. In particular, there is a parameter $B$ that describes the beam width, and what this implies is that the Beam Search will consider $B$ tokens at a time when creating an output sequence. 
\begin{figure}[h]
  \centering
    \begin{tikzpicture}
    \node[blue!50] (a0) at (0,0) {$a^{\langle 0 \rangle}$};
    ...
    \draw[->] (g2) -- (4,0);
    \node[draw,square,orange] (gy1) at (4.25,0) {};
    \node[above = 1 mm of gy1,orange] (y1) {$\hat y^{\langle 1 \rangle}$};
  \end{tikzpicture}
  \caption{\footnotesize At the first step of Beam search, we feed our input sequence through the encoder and run the resulting output vector through a softmax over $|V|$ possible outputs, remembering the $B$ most likely words that might start off our output sequence; beam search will then store in memory these top $B$ possible choices.}
\end{figure}

We input our French sentence through an encoder network and then run a softmax over $|V|$ possible outcomes and keep track of the $B$ most likely tokens. We estimated $\Pr(y^{\langle 1 \rangle}|x)$ in doing so. Now, for each of these $B$ possible starting tokens, we then hardwire a choice of $y^{\langle 1 \rangle}$ into the first stage of our decoder network and allow it to run a softmax over $|V|$ possible words that could follow in the output sequence. Notice that we have $\mathcal O(B \cdot |V|)$ work to do here, since for each of the $B$ possible first words we are determining the most likely second word in the output sequence. Our network effectively tells us $\Pr(y^{\langle 2 \rangle} | x, \hat y^{\langle 1 \rangle})$, and by rules of conditional probability we can calculate
\begin{equation}
  \label{eq: beamsearch}
  \Pr(y^{\langle 1 \rangle}, y^{\langle 2 \rangle} | x) = \Pr(y^{\langle 1 \rangle} | x) \Pr(y^{\langle 2 \rangle} | x, y^{\langle 1 \rangle}).
\end{equation}
Note that the left hand side of the equation above is ultimately the quantity that we're interested in maximizing and using as a prefix to initialize the remainder of our search.
We compute this for all possible $B \cdot |V|$ choices of our second word, sort the probabilities from equation \ref{eq: beamsearch} and choose the $B$ most likely subsequences to form the start of our output sequence. 

To be concrete, suppose we had chosen $B = 3$ and that after running our input French sentence through the encoder, the softmax unit instructed us to remember the words ``in'', ``Jane'', and ``September'' as the most likely. For each of these words, we hardwire them to $\hat y^{\langle 1 \rangle}$ and then forward propagate through our entire network to predict $\hat y^{\langle 2 \rangle}$.
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \node[blue!50] (a0) at (0,0) {$a^{\langle 0 \rangle}$};
    ...
    \node[draw,square,orange] (gy1) at (4.25,0) {};
    \node[above = 1 mm of gy1,orange] (y1) {$\hat y^{\langle 1 \rangle}$};
    \node[above = 1 mm of y1] {in};
    \node[draw,square,orange] (gy2) at (5.5,0) {};
    \node[above = 1 mm of gy2,orange] (y2) {$y^{\langle 2 \rangle}$};
    \draw[->,orange] (gy1) -- (gy2);
    \draw[->,purple!50] plot [smooth] coordinates { (4.5, 0.8) (4.8, 1.15) (5.1, -1) (5.45, -0.35) };
\end{tikzpicture}
\caption{\footnotesize We show the second step of Beam search for our example in which ``in'', ``Jane'' and ``September'' were deemed the most probable words to lead the output sequence. The second step (and each subsequent step) will involve creating $B$ separate copies of our network and running our candidate words through. In this diagram, we've hardwired ``in'' as the first word in the output sequence, and forward propagation and running through another softmax layer over $|V|$ possible outputs will allow us to determine $\Pr(y^{\langle 2 \rangle} | x, \hat y^{\langle 1 \rangle})$ for each word in our vocabulary. We repeat this process for our remaining words ``Jane'' and ``September'', procuring a listing of $B \cdot |V|$ probabilities which we can use to compute
$\Pr(y^{\langle 1 \rangle}, y^{\langle 2 \rangle} | x)$; we then take the $B$ most likely and continue on, having saved these intermediate probabilities for future use. Note that it's possible that the $B$ most likely prefixes for our output sequence do not include all or each of the $B$ words we started with as candidates for $\hat y^{\langle 1 \rangle}$; if this happens, this implicitly means a word was rejected as starting the output sequence.}
\end{figure}

Suppose that our Beam search indicated that the three most likely sentence prefixes were ``in September'', ``Jane is'', and ``Jane visits''. We'll proceed to create $B$ copies of our RNN and use these to estimate $\Pr(y^{\langle 3 \rangle} | x, y^{\langle 1 \rangle}, y^{\langle 2 \rangle})$. Hopefully, we end up with an output sequence that is quite likely and ultimately ends in an end-of-sentence token. Notice that if we set $B=1$, we recover our greedy search algorithm; but by considering $B > 1$ we'll be more likely to find a better output sequence than what a greedy algorithm would give us.

\subsubsection{Refinements to Beam Search}
\paragraph{Length Normalization} This small change allows us to obtain much better results with beam search. Beam search seeks to maximize the following probability:
\begin{equation}
  \label{eq: lengthnorm}
  \argmax_y \prod_{t=1}^{T_y} \Pr(y^{\langle t \rangle} | x, y^{\langle 1 \rangle}, \ldots, y^{\langle t-1 \rangle})
\end{equation}
where the product of probabilities is really representing the probability of the entire output sequence conditional on the input $x$, i.e. it is equivalent to
maximizing $\Pr(y^{\langle 1 \rangle}, \ldots, y^{\langle T_y \rangle})$ by definitions of conditional probability. Notice that each probability is some number in the unit interval, but that most of our probabilities are small; multiplying many tiny floating points together increases our chance of numerical underflow, i.e. that the floating point representation is insufficient to store our resulting value accurately. Instead of maximizing a product of probabilities, we instead use logarithms.

\begin{equation}
  \label{eq: loggedlengthnorm}
  \argmax_y \sum_{y=1}^{T_y} \log \Pr(y^{\langle t \rangle} | x, y^{\langle 1 \rangle}, \ldots, y^{\langle t-1 \rangle})
\end{equation}

By taking logarithms, our algorithm becomes more numerically stable and less prone to rounding errors or numerical underflow. Since the logarithm function is strictly monotone increasing, maximizing \ref{eq: loggedlengthnorm} will give the same result as maximizing \ref{eq: lengthnorm}

There's one more change we can also make to improve our machine translation algorithm. Our original objective \ref{eq: lengthnorm} involves multiplying more numbers less than one as the output sequence grows in length: this means that longer sentences become quite unlikely, or put differently, that our objective function prefers short translations and output sequences. The same phenomena holds true for \ref{eq: loggedlengthnorm} in that since each probability lies in the unit interval, taking the logarithm yields a negative value, and so adding more negative values (for longer output sequences) decreases the objective function we are trying to maximize. Therefore, we modify our objective:

\[
  \frac{1}{T_y} \argmax_y \sum_{y=1}^{T_y} \log \Pr(y^{\langle t \rangle} | x, y^{\langle 1 \rangle}, \ldots, y^{\langle t-1 \rangle})  
\]
This reduces the penalty for longer translation. In practice, as a heuristic instead of dividing by $T_y$ we might try scaling by $\frac{1}{T_y^{\alpha}}$ for some $\alpha \in [0, 1]$; if $\alpha = 1$ then we normalize by length, and if $\alpha = 0$ we do not apply any normalization. Here, $\alpha$ becomes another hyperparameter that we can tune to try and obtain better results.
Although there is little theoretical justification for this, many groups have found it works well in practice.

\paragraph{Summarizing beam search} Suppose we run beam search for 30 steps, i.e. we consider output sequences up to 30 tokens in length. If we had a beam width of three, then at each step we would keep track of the top three possibilities for each possible sentence length in the set $\{1, 2, \ldots, 30\}$. We would then look at the output of all sentences and score them against our objective function, choosing the one which achieves the highest value on the normalized log-probability functional form.

\paragraph{Choosing Beam Width?} How do we choose the beam width $B$? The larger $B$ is, the more possibilities we're considering, and there is an increased chance that we'll find a better translation. But, the larger $B$ is, the more computationally expensive our algorithm becomes, because we have to keep track of all these different possible output sequences we could be emitting. If the beam width is very large, then you consider more possibilities but our algorithm is slower, and the memory requirements grow. If you use a beam width that is very small, you get a worse result because you're keeping in mind fewer possibilities as the algorithm is running. But, you're getting a result faster and the memory requirements are lower. In production systems, it's not uncommong to see a beam width of maybe 10; 3 is quite small and a beam width of 100 would be considered very large for a production system. For research applications where people are focused on squeezing out each extra bit of performance, it's not uncommon to see beam widths up to several thousand. There are often diminishing returns as $B$ grows very large. Beam search is not guaranteed to return the exact maximum for our objective function.

\subsubsection{Error analysis in beam search} We've previously seen how error analysis can help you focus your time on the most useful work for your project. Since Beam Search is an approximation algorithm, it doesn't necessarily return the best possible output sequence. What if our algorithm makes a mistake? It's worth knowing whether beam search was the culprit, or if it was our RNN model architecture. Continuing with our example, ``Jane visite l'Afrique en septembre'', suppose that a human provided the following translation as part of a training example: ``Jane visits Africa in September''. Let us denote this translation by a human by $y^*$, since it's our target reference. Now, suppose our algorithm outputs $\hat y$ as ``Jane visited Africa in September''. This is a much worse translation since it changes the fundamental meaning of the sentence. Now, if we have constructed an encoder-decoder network and we are also using beam search to output translations, wouldn't it be nice if we could know which of these two components is responsible for the error? Just like it may be tempting to collect more training data as this typically doesn't hurt the performance of our algorithm, it could also be tempting to expand the beam search width; but in the same way that getting more training data doesn't always get us the performance that we'd like, the same is true for increasing the beam width hyperparameter.

The most useful thing we can do is use our RNN network to compute $\Pr(y^*|x)$ and compare it with $\Pr(\hat y | x)$.
\begin{itemize}
\item If $\Pr(y^*|x) > \Pr(\hat y|x)$, our Beam Search algorithm failed to find   the best $\argmax_y$.
\item If $\Pr(y^*|x) \leq \Pr(\hat y|x)$, then our RNN model is at fault in the sense that it doesn't recognize the hand-crafted translation is superior to what it output.
\end{itemize}
Note that if we're using some form of length normalization, we should be comparing the values our optimization objective function takes on rather than the raw probabilities. So in going through an error analysis, we'd go through a development set and find examples of mistakes made by our algorithm. We'd then construct a table:
\begin{table}
  \begin{tabular}{l | l | c | c | c}
    Human translation & Algorithm output & $\Pr(y^*|x)$ & $\Pr(\hat y | x)$ & At                                                           fault? \\
    \hline
    Jane visits Africa in September & Jane visited Africa in September & $2 \times 10^{-10}$ & $1 \times 10^{-10}$ & Beam search
  \end{tabular}
  \caption{\footnotesize An example entry we might create when examining errors our algorithm made on the development set. Through this process, we can determine what fraction of errors are made due to beam search vs. the RNN model. It's only if we find that beam search is responsible for a sizeable proportion of errors that we'd want to increase beam width. Else, we find the RNN is perhaps at fault most of the time and we could go through a deeper level of analysis to determine whether to add regularization, obtain more training data, or try a different network architecture, or something else.}
\end{table}

Error analysis like this is particularly helpful when we have an aproximation algorithm such as Beam Search, that is working to optimize some objective function.

\subsubsection{\href{https://www.aclweb.org/anthology/P02-1040.pdf}{Bleu} Score} One of the challenges with machine translation is the fact that there can be multiple valid translations, each equally good. How can we evaluate a machine translation system when there are several equally valid translations? Unlike an image recognition system where there is only one right answer, in which case we can measure accuracy, the way to measure performance when there are multiple correct answers is through something called a BLEU score, a Bi-lingual Evaluation Understudy.\footnote{In the context of theatre, the understudy is someone who learns the role of a more senior actor so they can take over if necessary. The motivation for BLEU is that, whereas we could ask human evaluators to evaluate the machine translation system, the BLEU score is an understudy, which could be a substitute for having humans evaluate every output of a machine translation system.}

Suppose we are given a French sentence
\begin{verbatim}
Le chat est sur le tapis.
\end{verbatim}
We are also given a reference, human generated: ``the cat is on the mat''. But a different human could have translated this as ``there is a cat on the mat'', which is just as acceptable. The BLEU score, allows us to automatically compute a score that measures how good is our machine translation. The idea is that so long as the machine generated translation is pretty close to any of the references provided by humans, then it should get a high BLEU score.

\paragraph{Why Precision is not a useful metric in machine translation}
In a bit more detail, we're going to look at the machine generated output and see if the types of words it generated appear in at least one of the human generated references which are provided as part of the dev and test set. To start, let's consider an extreme example. Suppose the machine translation is the word ``the'' repeated seven times over, which is clearly a pretty terrible translation. One way to measure the precision of the machine translation output would be to look at each word in the output sequence and see if it appears in the references. In our poor translation, the word ``the'' appears in both references; we're hitting $7/7 = 1$ and it appears that precision is not a particularly useful measure.

\paragraph{Modified precision}
Let's instead consider a modified precision measure, where we give each word credit only up to the maximum number of times it appears in the reference sentences. Since the word ``the'' appears up to two times in some of our reference translations, we're only going to allow up to two credits when calculating modified precision, which for our example would be $2/7$. The discussion thus far has been for calculating modified precision one word at a time, or for words in isolation.

\paragraph{BLEU Score Components}
For the BLEU score, we're interested in looking at bigrams as well, i.e. words appearing next to each other. Suppose our reference (human) translations are still ``the cat is on the mat'' and ``there is a cat on the mat'' but our machine translation is now ``the cat the cat on the mat''. Here, there are several possible bi-grams in the machine translation output:
\begin{table}[h]
  \centering
  \begin{tabular}{l| c c}
    & Count & $\textrm{Count}_{\textrm{Clipped}}$ \\
    \hline
    the cat & 2 & 1 \\
    cat the & 1 & 0 \\
    cat on  & 1 & 1 \\
    ...
    the mat & 1 & 1
  \end{tabular}
  \caption{\footnotesize In calculating $\textrm{Count}_{\textrm{Clipped}}$, we allow our algorithm to get credit only up to the maximum number of times that the bigram appears in any one of our human translations. Finally, our modified bi-gram precision will be the sum of $\textrm{Count}_{\textrm{Clipped}}$ divided by the total number of bi-grams (the sum of $\textrm{Count}$).}
\end{table}

Formally,

\[
p_1 = \frac{\sum_{\textrm{unigrams } \in \hat y} \texttt{Count}_{\texttt{clipped}}(\textrm{unigram})}{\sum_{\textrm{unigrams } \in \hat y} \texttt{Count}(\textrm{unigram})}
\]
where the subscript denotes uni-grams. We can also do this for $n$-grams:
\[
p_n = \frac{\sum_{\textrm{n-grams } \in \hat y} \texttt{Count}_{\texttt{clipped}}(\textrm{n-gram})}{\sum_{\textrm{n-grams } \in \hat y} \texttt{Count}(\textrm{n-gram})}.
\]
These modified precision scores allow us to measure the degree to which our machine translation overlaps with our human translation references. In the event that our machine translation is equal to either one of our references, we achieve $p_i$'s all equal to unit value. The combined BLEU score is given by
\[
\texttt{BP} \cdot \exp\left(\frac{1}{4} \sum_{n=1}^4 p_n\right)
\]
where \texttt{BP} stands for a brevity penalty. This is because with short translations, we have a higher chance of having the few words that do appear in our machine translation to also appear in the human reference translations. But, we don't want our translations to be unnecessarily short. The brevity penalty is an adjustment factor that looks like
\begin{equation*}
  \texttt{BP} = \begin{cases}
    1 & \textrm{ if } \texttt{Mach.Transl.}_{\textrm{output length}} >     \texttt{Ref.Transl.}_{\textrm{output length}} \\
    \exp\left(1 - \frac{\texttt{ReferenceTranslation}_{\textrm{output length}}}{  \texttt{MachineTranslation}_{\textrm{output length}}}\right) & \textrm{ otherwise}
  \end{cases}
\end{equation*}
We've previously seen the importance of having a single real number evaluation metric. Although BLEU is by no means perfect, it does give us a yardstick and that can enable progress to be measured. BLEU can be used not only in evaluating machine translations, but also in assessing the performance of image captioning systems: we would have a neural network generate a translation and then use a BLEU score to see how much of that caption overlaps with captions generated by people. Note that BLEU is only applicable when there are multiple equally good options for the output sequence; it does not apply to speech recognition since there we are trying to get an output sequence that is word for word correct, and there isn't a notion of multiple equally good outputs.

\subsubsection{\href{https://arxiv.org/abs/1409.0473}{Attention Model} Intuition} Up until now, we've been using an Encoder-Decoder network architecture to enable machine translation, where one RNN reads in the input sentence and another RNN outputs a sequence. The attention algorithm makes this work much better. To see why, suppose we're given a very long French sentence

\begin{quotation}
  Jane s'est rendue en Afrique en septembre dernier, a appr\'{e}ci\'{e} la culture et a rencontr\'{e} beaucoup de gens merveilleux; elle est revenue en parlant comment son voyage \'{e}tait marveilleux, et elle me tente d'y aller aussi.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    \node[blue!50] (a0) at (0,0) {$a^{\langle 0 \rangle}$};
    \node[blue!50,draw,square] (enc1) at (1,0) {};
    \node[blue!50] (x1) at (1,-1) {$x^{\langle 1 \rangle}$};
    \draw[blue!50,->] (a0) -- (enc1);
    \draw[blue!50,->] (x1) -- (enc1);
    \node[blue!50] (encdots) at (2,0) {$\ldots$};
    \node[blue!50,draw,square] (enc2) at (3,0) {};
    \node[blue!50] (xtx) at (3,-1) {$x^{\langle T_x \rangle}$};
    \draw[blue!50,->] (enc1) -- (encdots);
    \draw[blue!50,->] (encdots) -- (enc2);
    \draw[blue!50,->] (xtx) -- (enc2);
    \node[purple!50,draw,square] (dec1) at (4,0) {};
    \draw[blue!50,->] (enc2) -- (dec1);
    \node[purple!50] (decdots) at (5,0) {$\ldots$};
    \node[purple!50,draw,square] (dec2) at (6,0) {};
    \draw[purple!50,->] (dec1) -- (decdots);
    \draw[purple!50,->] (decdots) -- (dec2);
    \node[purple!50] (y1) at (4, 1) {$\hat y^{\langle 1 \rangle}$};
    \node[purple!50] (yty) at (6, 1) {$\hat y^{\langle T_y \rangle}$};
    \draw[purple!50,->] (dec1) -- (y1);
    \draw[purple!50,->] (dec2) -- (yty);
    \draw[->,purple!50] plot [smooth] coordinates { (4.2, 0.8) (4.3, 0.8) (4.4, -.4) (4.5, -.4) (4.6, -.2)};
    \draw[->,purple!50] plot [smooth] coordinates { (5.4, 0.8) (5.5, 0.8) (5.6,       -.4) (5.7, -.4) (5.8, -.2)};
\end{tikzpicture}
\caption{\footnotesize Suppose we have a very long input sequence. We are asking the encoder to read in the entire sequence and memorize it in the form of stored activations. Realize that a human translator would likely \emph{not} first read the entire French sentence, memorize it, then regurgitate an English sentence from scratch. Instead, they're more likely to read the first part of the sentence, maybe generate part of the translation, then look at the second part of the input sequence, generate a few more words, and so on. It's difficult for a human to memorize a whole long sentence like that. Although the encoder-decoder network we've started with works well for short (less than 40 words) sequences, it works less well for longer sequences; i.e. the BLEU score is expected to drop off as a function of sentence length. With attention models, we can translate sentences in fragments and this allows us to achieve consistently high BLEU scores even as sentence length increases.}
\end{figure}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \path[->,orange!50] (af2) edge[bend left] (af3);    
    \path[->,orange!50] (af3) edge[bend left] (af4);
    \path[->,orange!50] (af4) edge[bend left] (af5);
    \path[->,purple!50] (ab5) edge[bend left] (ab4);
    \path[->,purple!50] (ab4) edge[bend left] (ab3);
    \path[->,purple!50] (ab3) edge[bend left] (ab2);
    \path[->,purple!50] (ab2) edge[bend left] (ab1);
    \node at (3, -2) {jane};
    \node at (3*2, -2) {visite};
    \node at (3*3, -2) {l'Afrique};
    \node at (3*4, -2) {en};
    \node at (3*5, -2) {septembre};
    \node (c1) at (3, 2) {c};
    \node (s0) at (2, 3) {$s^{\langle 0 \rangle}$};
    \node[draw,square] (s1) at (3, 3) {$s^{\langle 1 \rangle}$};
    \node[above=1cm of s1,draw,rectangle] (yhat1) {jane};
    \draw[->] (s0) -- (s1);
    \draw[->] (c1) -- (s1);
    \draw[->] (s1) -- (yhat1);
    \foreach \y in {1, ..., 5} {
      \draw[->,dashed] (y\y) -- (c1);
      \node[right = 1mm of y\y] {$\alpha_{1,\y}$};
    }
  \end{tikzpicture}
  \caption{\footnotesize Here, we use a bi-directional RNN. Note that we're not doing a word-for-word translation, and so the length of the output sequence need not equal the length of the input sequence. But by using a BRNN, we've effectively created a rich set of features for each word in the input sequence and perhaps surrounding words in the sentence. To generate the English translation, we'll use another RNN, but to avoid confusion with the activations in our encoder BRNN, we will use $s^{\langle i \rangle}$ to denote the activation for the $i$th layer. The hope is that the first output in the sequence will be ``Jane'', but when the network is trying to generate the first word, what part of the input sentence should it be looking at? Seems reasonable that we should be looking at the first word and nearby words, but that we don't need to look toward the very end of the sentence. The attention model computes a set of attention weights $\alpha_{i,j}$ which tell us how much weight should we give the $j$th word in the input sequence when determining the $i$th word in the output sequence. Ultimately, these weights create a context vector $c$ which is fed as input to the first building block of our decoder RNN.}
\label{fig: machinetranslationattentionmodel1}
\end{figure}

Diagram \ref{fig: machinetranslationattentionmodel1} depicts just one step of the RNN for our machine translation task.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \draw[->] (c1) -- (s1);
    \draw[->] (s1) -- (yhat1);
    \node[draw,square,purple!50] (s2) at (6, 3) {$s^{\langle 2 \rangle}$};
    \draw[->,purple!50] plot [smooth] coordinates { (3.5, 4.5) (4,4) (4.5, 2.5) (5.5, 2.5)};
    \node (c2) at (6, 2) {$c$};
    \node[above = 1 cm of s2,draw,rectangle,purple!50] (yhat2) {visits};
    \draw[->,purple!50] (s2) -- (yhat2);
    \foreach \y in {1, ..., 5} {
      \draw[->,dashed,purple!50] (y\y) -- (c2);
      \node[right = 1mm of y\y,purple!50] {$\alpha_{2,\y}$};
    }
    \draw[->,purple!50] (c2) -- (s2);
    \draw[->,purple!50] (s1) -- (s2);
  \end{tikzpicture}
  \caption{\footnotesize At the second step, we will have a new hidden state $s^{\langle 2 \rangle}$, a context vector $c$ that depends on $\alpha_{2,j}$ for $j \in \{1, \ldots, T_x\}$ which tells us how much weight to place on the $j$th word when outputting the second word in the output sequence. We'll also feed the $\hat y_1$ as input, which in this case hopefully is the word (or rather name) ``Jane''.}
\label{fig: machinetranslationattentionmodel2}
\end{figure}

Note that in figure \ref{fig: machinetranslationattentionmodel2}, at this point we still haven't defined exactly how to create the context vector $c$, as well as how to compute the attention weights $\alpha_{i,j}$. In general, $\alpha_{i,j}$ depends on the activations from the BRNN in the input sequence at that time-step $t$, $\overrightarrow a^{\langle t \rangle}$ and $\overleftarrow a^{\langle t \rangle}$ as well as the state from previous steps which is embedded in $s^{\langle t \rangle}$.

The key is that the RNN marches forward one step at a time until we eventually generate an EOS token. At each step when we're trying to generate the $t$-th word in the output sequence, we have a set of attention weights that tell us how much weight to place on the $t'$-th words in the input sequence; perhaps this informs us to only look within a local window within the input sequence in order to determine the correct output translation word.

\subsubsection{Attention Model} The attention model allows a neural network to pay attention to only part of an input sequence when generating a single word in the output translation. Let's now focus on the implementation details. Let's assume we're using a BRNN, with either GRU's or LSTM's as building blocks, for our encoder network; this is our means for computing features for word in the input sequence. Note that since we have a bi-directionall RNN, we have a set of forward \emph{and} backward activations for each word in the input sequence. For simplicity of notation, in this section only let us denote by $a^{\langle t' \rangle} = \left( \overrightarrow a^{\langle t' \rangle}, \overleftarrow a^{\langle t' \rangle} \right)$ i.e. we will consider both forward and backward activations and concatenate them into our features for the $t$th word in the input sequence. So, $a^{\langle t' \rangle}$ will denote the feature vector at time step $t'$. Here, our convention is that we use a prime suffix (e.g. $t'$ instead of $t$) to index into our input sequence.

\paragraph{Calculating a context vector}
Critically, our decoder network will be a single-directional RNN using hidden state $s$ to generate the translation. We will feed in $s^{\langle 0 \rangle}$ along with a context vector $c$, generated using attention parameters $\alpha_{1j}$ for $j \in \{1, \ldots, T_x\}$. The way we define context is by a weighted sum where the weights are given by the attention weights. Note that we require attention weights are non-negative and sum to one.
\begin{align*}
  \alpha^{\langle t, t' \rangle} \in [0,1] \hspace{5pt} \forall t, t' \hspace{35pt} \sum_{t'} \alpha^{\langle 1, t'\rangle} = 1 \hspace{25pt} \leadsto \hspace{25pt}
  c^{i} = \sum_{t'} \alpha^{\langle i, t' \rangle} a^{\langle t \rangle}
\end{align*}
where again $\alpha^{\langle t, t' \rangle}$ is the amount of attention that $y^{\langle t \rangle}$ should pay attention to $a^{\langle t' \rangle}$.

\paragraph{Choosing attention weights} Let's start with the formula.
\[
  a^{\langle t, t' \rangle} = \frac{\exp\left(e^{\langle t, t' \rangle}\right)}{\sum_{t'=1}^{T_x} \exp\left(e^{\langle t, t' \rangle}\right)}
\]
i.e. for each fixed value of $t$, if we sum over $t'$ we get unit value as output.

\begin{figure}[h]
  \centering
  \begin{tikzpicture}
    \node (a0) at (1.5,0) {$a^{\langle 0 \rangle}$};
    \foreach \x in {1,...,4} {
      \node[draw,rectangle,scale=1.5] (af\x) at (3*\x-0.65,0) {};
      \node[draw,rectangle,scale=1.5] (ab\x) at (3*\x+0.65,0) {};
      \node (y\x) at (3*\x,  1) {}; %{$\hat y^{\langle \x \rangle}$};
      \draw[->] (x\x) -- (af\x);
      \draw[->] (x\x) -- (ab\x);
      \draw[->] (af\x) -- (y\x);
    ...
    \draw[->] (a0) -- (af1);
    \node (dots) at (3*2.5, 0) {$\ldots$};
    \node (x1) at (3*1, -1) {$x^{\langle 1 \rangle}$};
    \node (x2) at (3*2, -1) {$x^{\langle 2 \rangle}$};
    \node (xtxm1) at (3*3, -1) {$x^{\langle T_x - 1 \rangle}$};
    \node (xtx) at (3*4, -1) {$x^{\langle T_x \rangle}$};
    \node (oplus) at (3*2.5, 2.5) {$\oplus$};
    \draw[->] (y1) -- (oplus);
    \draw[->] (y2) -- (oplus);
    \draw[->] (y3) -- (oplus);
    \draw[->] (y4) -- (oplus);
    \node[above=1cm of oplus, draw, square] (st) {$s^{\langle t \rangle}$};
    \node[left =1cm of st,    draw, square] (stm1) {$s^{\langle t - 1         \rangle}$};
    \draw[->] (stm1) -- (st);
    \node[above=1cm of stm1] (ytm1) {$\hat y^{\langle t - 1 \rangle}$};
    \draw[->] (stm1) -- (ytm1);
    \node[above=1cm of st] (yt) {$\hat y^{\langle t \rangle}$};
    \draw[->] (oplus) -- (st);
    \draw[->] (st) -- (yt);
    \draw[->,purple!50] plot [smooth] coordinates { (6.25, 6) (6.5, 6) (6.5, 3.5) (7, 4)};    
    \path[->,orange!50] (af1) edge[bend left] (af2);    
    % \path[->,orange!50] (af2) edge[bend left] (af3);    
    \path[->,orange!50] (af3) edge[bend left] (af4);
    \path[->,purple!50] (ab4) edge[bend left] (ab3);
    ...
    \path[->,purple!50] (ab2) edge[bend left] (ab1);
  \end{tikzpicture}
  \caption{\footnotesize Where to calculate $e^{\langle \cdot, \cdot \rangle}$, we can actually use a small neural network as follows, recalling that $s^{\langle t-1 \rangle}$ is the hidden state from time-step $t-1$. It seems reasonable that $\alpha^{\langle t, t'\rangle}$ should depend on the previous hidden state of our output sequence as well as the activation features from our input sequence. Note that we haven't yet calculated the context vector yet since this depends on the weights $\alpha^{\langle \cdot, \cdot \rangle}$. Although we have an intuition as to what $\alpha^{\langle t, t'\rangle}$ should depend on, we don't know \emph{how} it depends on these parameters, so why not let a small neural network figure it out for us.}
\end{figure}

\paragraph{Computational Complexity} One downside of this algorithm is that its runtime is quadratic: if you have $T_x$ words in the input and $T_y$ words in the output, then the total number of attention parameters we need to set are $T_x \times T_y$. It's worth noting that in machine translation, each sentence isn't necessarily too long, and so perhaps quadratic complexity is acceptable.

\paragraph{\href{https://arxiv.org/abs/1502.03044}{Image Captioning}} The task is to look at a picture and generate a caption describing it. It turns out we can also use the attention here: simply look at the picture and only pay attention to parts of the picture at a time when generating a caption.

\subsection{Speech Recognition - Audio Data}
\subsubsection{Speech Recognition} One of the most exciting developments with sequence-to-sequence models has been the rise of very accurate speech recognition systems. The speech recognition problem is that you're given an audio clip $x$ and you're asked to produce a transcript $y$. What a microphone really does is measure miniscule changes in air pressure; an audio clip plots air pressure against time.

\begin{figure}[h]
  \centering
  \includegraphics[scale=0.35]{fig/audio_waveform}
\end{figure}

Realize that the human ear does not process raw waveforms, but instead has physical structures that measures the amount of intensity of different frequencies. A common pre-processing step for audio data is to run your raw audio clip and generate a \emph{spectrogram}: a plot where the horizontal axis is time, the vertical axis is frequencies, and the colors at a grid location show the amount of energy. I.e. how loud is the sound at different frequences and at different times. Once upon a time, speech recognition systems were built using phonemes and these were hand-engineered basic units of sounds. So, for example, ``the quick brown'' can be represented using phonemes, something like ``de kwik brawn''. But with end-to-end deep learning, we're realizing t hat phoneme representations are no longer necessary: instead we can build systems that input an audio clip and directly output a transcript without needing to use hand-engineered representations like these mentioned above. Critically, this was made possible by moving toward much larger datasets; 3,000 hours of transcribed audio might be considered reasonable for a research application, but today the best commercial systems are trained on sometimes over 100k hours of audio.

In the last video, we discussed the attention model. This can be applied here as well. One method that works well is \href{https://www.cs.toronto.edu/~graves/icml_2006.pdf}{Connectionist Temporal Classification}. Suppose the audio clip is of someone saying ``the quick brown fox''. We're going to use an RNN where $T_x = T_y$; we can use a bi-directional RNN with GRU's or LSTM's and we can make our BRNN deep in practice, but for simplicity sake we will just draw out a uni-directional RNN and abstract away details. What's unique about audio recognition problems is that the number of input time steps is much bigger than the number of output time steps. E.g. if you have 10 seconds of audio and your features come in at 100 hertz i.e. 100 samples per second, we end up with 1k inputs. But your output may not have 1k characters in the vocabulary. The CTC cost function allows the RNN to generate an output like follows
\begin{verbatim}
ttt_h_eee___ ___qqq
\end{verbatim}
where the \texttt{\_} represents a special blank character and the previous snippet would be considered a correct output for our example input. Note that we have a space separating ``the'' from the ``q'', and this is a separate character from the special blank character \texttt{\_}. The basic rule for the CTC cost function is to collapse repeated characters not separated by ``blank''. The above output gets collapsed to ``the q''. This allows the neural network to have thousands of outputs by repeating characters multiple times, inserting a bunch of blank characters, and we still end up with a much shorter output text transcript.

\subsubsection{Trigger word detection} There are many devices that can be ``woken up'' with your voice, and this is known as a trigger word detection system; let's see how we can build one. We start by taking an input audio clip, pre-processing to produce spectrogram features, and then pass these through an RNN. It remains to define the target labels $y$. If at a particular point in the audio clip an individual has just finished stating the trigger word, we can label everything before that point in the clip a zero and a one right after that. This type of labeling scheme \emph{could} work. A slight disadvantage is that it creates a very imbalanced training set: there are a lot more zeros than ones. One thing you could do is instead of setting a single timestamp equal to one when a wake word is mentioned is to set multiple timestamps equal to one before reverting back to zero. This can slightly even out the ratio of ones to zeros, but this is more or less a hack.

\section{Summary} Deep learning is like a superpower. We can make a computer see, have a computer synthesize a novel or music. We can have a computer translate from one language to another. We can make even look at a radiology image and render a medical diagnosis. The word today has challenges, but with AI we can potentially make the world a much better place.
