...
\begin{tikzpicture}   \node (hlp) at (0, 0) {\textrm{Human level performance}};
  ...
  \draw[<->] (hlp) -- (train) node[midway,right] {\textrm{avoidable bias}};
  \node[below = 0.5 cm of train] (traindev) {\textrm{\texttt{Training-dev} set error}};
  \draw[<->] (train) -- (traindev) node[midway,right] {\textrm{variance}};
  \node[below = 0.5 cm of traindev] (dev) {\textrm{Dev error}};
  \draw[<->] (traindev) -- (dev) node[midway,right] {\textrm{data mismatch}};
  \node[below = 0.5 cm of dev] (test) {\textrm{Test error}};
  \draw[<->] (dev) -- (test) node[midway, right] {\textrm{over-fitting to dev set}}; \end{tikzpicture}
