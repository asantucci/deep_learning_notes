...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \draw[->] (6.25, 2.25) -- (6.25, 2.75) node[above] {$\hat y^{\langle T_y \rangle}$};    
  \end{tikzpicture}
  \caption{\footnotesize An example model architecture for music generation. The input could be an integer describing the genre, or perhaps the first note of the desired output melody. We make a sequence of $T_y$ predictions, where only the first depends on an input $X$.}
\end{figure}
